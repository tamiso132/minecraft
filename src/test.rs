...
impl ApplicationTrait for TestApplication {
    fn on_new(event_loop: &EventLoop<()>) -> Self {
        ...
        let imgui_values = ImguiRender { pipeline_index: 0, is_frustum: false };
    ...
    fn on_draw(&mut self) {
        unsafe {
            ...
            let pipeline = self.pipeline[self.imgui_values.pipeline_index as usize];
            ...
            self.imgui_values.render_imgui(ui);
